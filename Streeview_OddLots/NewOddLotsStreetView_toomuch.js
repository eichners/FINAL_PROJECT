...
//function addBufferedData() {
 $.getJSON('https://eichnersara.cartodb.com/api/v2/sql?q=SELECT ST_Transform(ST_Buffer(the_geom_webmercator, 100), 4326) AS the_geom FROM oddlots_brooklyn &format=GeoJSON')
   .done(function (data) {
    console.log(data);
    var bufferData = data;
...
function addGroupData() {
  $.getJSON('https://eichnersara.cartodb.com/api/v2/sql?q=SELECT ST_Transform(ST_Convexhull(ST_Collect(the_geom_webmercator)), 4326) AS the_geom FROM oddlots_brooklyn GROUP BY schooldist &format=GeoJSON')
  .done(function (data) {
    var groupData = data;
    console.log(data);
